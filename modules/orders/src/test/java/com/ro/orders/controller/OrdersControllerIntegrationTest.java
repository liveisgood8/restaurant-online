...
class OrdersControllerIntegrationTest extends AbstractControllerTest {
  ...
  @Test
  void makeOrderByCash_whenUnauthorized_thenMustBeNonApproved() throws Exception {
    ...
    assertMakeOrderAnswer(givenOrder, result);
    assertOrderInDatabase(givenOrder, false);
  }
  ...
  private OrderDto createOrderDtoForMakeOrder(String paymentMethodName) {
    ...
    OrderDto orderDto = er.nextObject(OrderDto.class);
    orderDto.setId(null);
  ...
  private OrderPartDto createOrderPartDto() {
    ...
    OrderPartDto partDto = er.nextObject(OrderPartDto.class);
  ...
  private Dish createAndSaveDish() {
    ...
    Dish dish = er.nextObject(Dish.class);
    dish.setCategory(category);

    return dishRepository.save(dish);
  }

  private void assertMakeOrderAnswer(OrderDto givenOrder, ResultActions resultActions) throws Exception {
      resultActions
  ...
  private void assertOrderInDatabase(OrderDto givenOrder, boolean isUserWasAuthenticatedOnRequest) {
    inTransaction(() -> {
      ...
      assertEquals(givenOrder.getPaymentMethod().equals(PaymentMethod.BY_CARD_ONLINE), order.getIsApproved());
      if (!isUserWasAuthenticatedOnRequest) {
